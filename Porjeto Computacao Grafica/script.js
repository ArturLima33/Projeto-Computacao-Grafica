...
function drawHead(){
    let relMouseX = mouseX - (canvas.width/2)
    let relMouseY = mouseY - (canvas.height/2)
    let ang = Math.atan2(relMouseY, relMouseX) * 0.3

    ctx.save()
    ctx.translate(0, -40)
    ctx.rotate(ang)
    ctx.fillStyle="#bdc3c7"
    ctx.fillRect(-24,-50,48,40)
    ctx.fillStyle="#00e5ff"
    ctx.fillRect(-16,-38,32,14)
...
function drawArm(xOffset){
    let relMouseX = mouseX - (canvas.width/2)
    let relMouseY = mouseY - (canvas.height/2)
    let ang = Math.atan2(relMouseY, relMouseX)

    ctx.save()
    ctx.translate(xOffset, -35)
    ctx.rotate(ang)
    ctx.fillStyle="#95a5a6"
    ctx.fillRect(0,-10,40,20)
    ctx.translate(40, 0)
    ctx.rotate(Math.sin(walkTime)*0.2)
    ctx.fillStyle="#7f8c8d"
...
function drawLeg(x,side){
    let swing = Math.sin(walkTime) * 20
...
function draw(){
    ...
    ctx.save()
    ctx.translate(robotX, (isJumping ? groundY : robotY) + 105)
    ctx.scale(1.2, 0.3)
    ...
    let bounce = 1 + Math.sin(walkTime)*0.02
